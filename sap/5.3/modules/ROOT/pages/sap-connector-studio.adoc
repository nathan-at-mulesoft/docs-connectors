= SAP Connector 5.3 Studio Configuration - Mule 4
...
. Click *Global Elements* at the base of the Mule flow canvas, and click *Create*.
. In the *Connector Configuration*, select *SAP Config*.
...
|Login Language | In the *Advanced* tab, specify the language to use for the SAP connection. `EN` for English is the default.
|===

As a best practice, use property placeholder syntax to load the credentials in a more simple and reusable way.

Click *Test Connection* to verify that the connection to the SAP instance is correct. If the credentials are correct, you see the `Test Connection Successful` message.
