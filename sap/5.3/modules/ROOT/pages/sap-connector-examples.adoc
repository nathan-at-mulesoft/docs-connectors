...
Create a Mule app that establishes a connection to SAP and receives a function using the SAP connector's *Function listener* operation, which returns the export parameter to SAP.
...
+
The flow now contains *Function listener* in the canvas source area and the *Transform Message*, as shown in the following image:
+
