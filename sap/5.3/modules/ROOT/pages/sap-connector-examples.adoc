...
Create a Mule app that establishes a connection to SAP and receives a function using the SAP connector's *Function listener* operation, which then returns the export parameter to SAP.

== Prerequisites

* xref:sap-connector-studio.adoc#install-sap-libraries[Install the SAP Libraries]

== Establish an SAP Connection

Create and configure a global element to establish a connection to SAP. 
...
. Click *Add*.
. Click *Finish*.
. Drag the SAP *Function listener* operation to the source area of the canvas at the start of a flow.
. In *Basic Settings*, click *+* to the right of *Connector configuration* to create a new SAP inbound global element configuration and specify the environment values.
. In the *General* tab, enter the required parameters (supplied by your SAP administrator) for defining an SAP connection.
. Add a *Transform Message* component to the flow and specify the details based on the metadata:
+
image::sap-function-return-response.png[Output payload of the return response]
+
The flow now contains the *Function listener* operation and *Transform Message* component in the canvas source area.
+
...
Create a Mule app to send an IDoc to SAP:
...
Use the same HTTP configuration that is used for the HTTP *Listener* operation.
...
. Select the *Send IDoc* operation next to the HTTP *Listener* operation.
...
. Drag the SAP *Document listener* operation to the start of a flow in the source area of the canvas.
. Create a new *SAP Inbound* global element configuration and specify the environment values.
. Add a *Logger* right after the *Document listener*. +
