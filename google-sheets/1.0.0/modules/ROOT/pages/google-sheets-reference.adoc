...
* <<GetFiles>> {nbsp}
* <<GetSpreadsheetsBySpreadsheetId>> {nbsp}
...
| Spreadsheet a| String |  Spreadsheet Id |  | *x*{nbsp}
...
| Order By a| String |  Order By |  | {nbsp}
...
Get Spreadsheet Properties This operation makes an HTTP GET request to the /spreadsheets/{spreadsheetId} endpoint
...
| Spreadsheet a| String |  The ID of the spreadsheet. |  | *x*{nbsp}
...
** HOURS
** DAYS |  |  SECONDS | {nbsp}
| Include Newly Added Rows a| Boolean |  Indicates whether to include also newly added rows in the output. |  false | {nbsp}
| Config Ref a| ConfigurationProvider |  The name of the configuration to be used to execute this component |  | *x*{nbsp}
| Primary Node Only a| Boolean |  Whether this source should only be executed on the primary node when runnning in Cluster |  | {nbsp}
| Scheduling Strategy a| scheduling-strategy |  Configures the scheduler that triggers the polling |  | *x*{nbsp}
| Streaming Strategy a| * <<RepeatableInMemoryStream>>
