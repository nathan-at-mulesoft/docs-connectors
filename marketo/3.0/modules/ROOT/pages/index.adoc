...
Anypoint Connector for Adobe Marketo (Marketo Connector) facilitates access to the Adobe Marketo REST APIs. This connector implements all supported v1.0 Adobe Marketo API endpoints and provides Mule 4 DataWeave functionality.
...
. In Flow Designer, select a trigger such as the HTTP Listener or Scheduler. (Adobe Marketo does not provide a trigger operation.)
